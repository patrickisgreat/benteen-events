...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between gap-3">
      ...
      <UButton label="New event" icon="i-lucide-plus" @click="openNew" />
    </div>

    <UAlert v-if="error" color="error" variant="subtle" icon="i-lucide-circle-alert" :description="error" />

    <div v-if="pending && !upcoming.length && !past.length" class="flex justify-center py-10">
    ...
    <template v-else>
      <section v-if="upcoming.length" class="space-y-3">
        ...
        <ul class="grid gap-3 sm:grid-cols-2">
          <li v-for="ev in upcoming" :key="ev.id">
            <EventSummaryCard :event="ev" @edit="openEdit(ev)" @delete="askDelete(ev)" />
          </li>
        </ul>
      </section>
      ...
      <section v-if="past.length" class="space-y-3">
        ...
        <ul class="grid gap-3 sm:grid-cols-2">
          <li v-for="ev in past" :key="ev.id">
            <EventSummaryCard :event="ev" @edit="openEdit(ev)" @delete="askDelete(ev)" />
          </li>
        </ul>
      </section>

      <UCard v-if="!upcoming.length && !past.length" variant="subtle" class="text-center">
        <div class="py-8 space-y-3">
          <UIcon name="i-lucide-calendar-plus" class="size-8 text-muted" />
          <p class="font-medium">
            No events yet
          </p>
    ...
    <EventFormModal v-model:open="formOpen" :event="editing" @saved="onSaved" />

    <UModal :open="pendingDelete !== null" title="Delete this event?" @update:open="cancelDelete">
      <template #body>
        <p class="text-sm text-muted">
        ...
        <div class="flex justify-end gap-2 mt-4">
          <UButton label="Cancel" color="neutral" variant="ghost" @click="cancelDelete" />
          <UButton label="Delete" color="error" :loading="deleting" @click="onConfirmDelete" />
        </div>
      </template>
    </UModal>
  </div>
